...
namespace sniff_proto {
    ...
    class SniffPluginSsh : public ISniffPlugin {
    public:
        static const std::string default_name;
        static const unsigned short default_port;

        SniffPluginSsh(std::string config, boost::function<create_config_parser> &parse_dll) : config_(config), parse_dll_(parse_dll) {}
        ...
        VIRTUAL_METHOD std::string CALL_METHOD get_name() {
            if (name_.empty()) {
                name_ = SniffPluginSsh::default_name;
            }
            return name_;
        }
        ...
        VIRTUAL_METHOD std::vector<unsigned short> CALL_METHOD get_ports() {
            if (ports_.empty()) {
                ports_.push_back(SniffPluginSsh::default_port);
            }
            return ports_;
        }
        ...
        virtual ~SniffPluginSsh() {}

        static boost::shared_ptr<SniffPluginSsh> create_sniff_plugin(std::string config, boost::function<create_config_parser> &parse_dll) {
            return boost::shared_ptr<SniffPluginSsh>(new sniff_proto::SniffPluginSsh(config, parse_dll));
        }
    ...
    const std::string SniffPluginSsh::default_name = "ssh";
    const unsigned short SniffPluginSsh::default_port = 22;

} // namespace sniff_proto


BOOST_DLL_ALIAS(sniff_proto::SniffPluginSsh::create_sniff_plugin, create_plugin)
