...
struct atomic_tag {
	...
	atomic_tag() {
		a = 0;
		std::cout << "atomic_tag" << std::endl;
	}
	atomic_tag(int a) {
		this->a = a;
		std::cout << "atomic_tag(int)" << std::endl;
	}
};
std::mutex mtx;
void test_atomic_custom() {
	std::cout << "-------------------------------" << __FUNCDNAME__ << std::endl;
	std::atomic<atomic_tag> atmtg(5);
	const int thread_nums = 5;
	std::thread ts[thread_nums];
	for (int i = 0; i < thread_nums; i++) {
		ts[i] = std::thread([&](int mils) {
			std::this_thread::sleep_for(std::chrono::milliseconds(mils));
			std::unique_lock<std::mutex> ul(mtx);
			atomic_tag atg = atmtg.load();
			atg.a += 1;
			atmtg.store(atg);
			atomic_tag atg2 = atmtg.load();
			atomic_tag atg3 = atmtg.load();
			//ul.unlock();
		},
			i * 10);
	}
	...
	std::cout << atmtg.load().a << std::endl;
}

void test_atomic_custom2() {
	std::cout << "-------------------------------" << __FUNCDNAME__ << std::endl;
	std::atomic<atomic_tag*> atmtg4(new atomic_tag(5));
	...
	for (int i = 0; i < thread_nums; i++) {
		ts[i] = std::thread([&](int mils) {
			std::this_thread::sleep_for(std::chrono::milliseconds(mils));
			std::unique_lock<std::mutex> ul(mtx);
			atomic_tag *atg = atmtg4.load();
			atg->a += 1;
			atmtg4.store(atg);
			atomic_tag *atg2 = atmtg4.load();
			atomic_tag *atg3 = atmtg4.load();
			//ul.unlock();
		},
			i * 10);
	}
	...
	std::cout << atmtg4.load()->a << std::endl;
}
...
void test_atomic() {
	...
	//         test_atomic_operator();
	//         test_atomic_store();
	//         test_atomic_flag_atomic_clear();
	test_atomic_custom();
	test_atomic_custom2();
}
//} // namespace atomic