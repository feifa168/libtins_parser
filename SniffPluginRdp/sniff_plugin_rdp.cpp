...
namespace sniff_proto {
    ...
    class SniffPluginRdp : public ISniffPlugin {
    public:
        static const std::string default_name;
        static const unsigned short default_port;

        SniffPluginRdp(std::string config, boost::function<create_config_parser> &parse_dll) : config_(config), parse_dll_(parse_dll) {}
        ...
        VIRTUAL_METHOD std::string CALL_METHOD get_name() {
            if (name_.empty()) {
                name_ = SniffPluginRdp::default_name;
            }
            return name_;
        }
        ...
        VIRTUAL_METHOD std::vector<unsigned short> CALL_METHOD get_ports() {
            if (ports_.empty()) {
                ports_.push_back(SniffPluginRdp::default_port);
            }
            return ports_;
        }
        ...
        virtual ~SniffPluginRdp() {}

        static boost::shared_ptr<SniffPluginRdp> create_sniff_plugin(std::string config, boost::function<create_config_parser> &parse_dll) {
            return boost::shared_ptr<SniffPluginRdp>(new sniff_proto::SniffPluginRdp(config, parse_dll));
        }
    ...
    const std::string SniffPluginRdp::default_name = "rdp";
    const unsigned short SniffPluginRdp::default_port = 3096;

} // namespace sniff_proto


BOOST_DLL_ALIAS(sniff_proto::SniffPluginRdp::create_sniff_plugin, create_plugin)
