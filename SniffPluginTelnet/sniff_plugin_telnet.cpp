...
namespace sniff_proto {
    ...
    class SniffPluginTelnet : public ISniffPlugin {
    public:
        static const std::string default_name;
        static const unsigned short default_port;

        SniffPluginTelnet(std::string config, boost::function<create_config_parser> &parse_dll) : config_(config), parse_dll_(parse_dll) {}
        ...
        VIRTUAL_METHOD std::string CALL_METHOD get_name() {
            if (name_.empty()) {
                name_ = SniffPluginTelnet::default_name;
            }
            return name_;
        }
        ...
        VIRTUAL_METHOD std::vector<unsigned short> CALL_METHOD get_ports() {
            if (ports_.empty()) {
                ports_.push_back(SniffPluginTelnet::default_port);
            }
            return ports_;
        }
        ...
        virtual ~SniffPluginTelnet() {}

        static boost::shared_ptr<SniffPluginTelnet> create_sniff_plugin(std::string config, boost::function<create_config_parser> &parse_dll) {
            return boost::shared_ptr<SniffPluginTelnet>(new sniff_proto::SniffPluginTelnet(config, parse_dll));
        }
    ...
    const std::string SniffPluginTelnet::default_name = "telnet";
    const unsigned short SniffPluginTelnet::default_port = 23;

} // namespace sniff_proto


BOOST_DLL_ALIAS(sniff_proto::SniffPluginTelnet::create_sniff_plugin, create_plugin)
